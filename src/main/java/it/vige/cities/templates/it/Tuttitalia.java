...
public class Tuttitalia extends Template {
		...
		for (Node node0 : nodes.getZones()) {
			for (Element head1 : lines1) {
				...
				if (associations.get(node0).contains(name)) {
					...
					node1.setId(counter++);
					node1.setLevel(1);
					node1.setName(normalize(caseSensitive, duplicatedNames, name,
					...
					for (Element head2 : lines2) {
						...
						node2.setName(normalize(caseSensitive, duplicatedNames, splittedName[1],
	...
	private int addLevel0(Nodes nodes) {
		...
		northWest.setName(normalize(caseSensitive, true, "I: ITALIA NORD-OCCIDENTALE", null));
		...
		northEast.setName(normalize(caseSensitive, true, "II: ITALIA NORD-ORIENTALE", null));
		...
		south.setId(counter++);
		south.setLevel(0);
		south.setName(normalize(caseSensitive, true, "III: ITALIA CENTRALE", null));
		...
		centre.setLevel(0);
		centre.setName(normalize(caseSensitive, true, "IV: ITALIA MERIDIONALE", null));
		...
		islands.setLevel(0);
		islands.setName(normalize(caseSensitive, true, "V: ITALIA INSULARE", null));
